...
class CCCList extends MusicBeatState
{
	...
	var ridzaka:Character = null;
	public static var isRidzak:Int = 0;
	...
	override public function create():Void
	{
		...
		leftArrow = new FlxSprite(1000, 470);
		...
		rightArrow = new FlxSprite(1200, 470);
	...
	override function update(elapsed:Float)
	{
		...
		if(controls.RESET)
			{
				if (optionShit[curSelected] == 'Anders')
					{
		...
		if (abBBB)
			...
			if(ridzakc != null && ridzakc.animation.curAnim.finished) {
				ridzakc.dance();
			}
			if(ridzaka != null && ridzaka.animation.curAnim.finished) {
				ridzaka.dance();
			}
	...
	function getLocked()
		{
			...
			if (FlxG.save.data.currentRidzak == 'minus' && FlxG.save.data.minusUnlocked != true)
				oppo = true;
			else if(FlxG.save.data.currentRidzak == 'alpha' && FlxG.save.data.alphaUnlocked != true)
				oppo = true;
			else if(FlxG.save.data.currentRidzak == 'classic' && FlxG.save.data.classicUnlocked != true)
				oppo = true;
			return oppo;
		}
		...
		function addRidzak()
			{
				...
				ridzakc = new Character(ridzakx-50, ridzaky-40, 'ridzak-pclassic', true);
				ridzakc.setGraphicSize(Std.int(ridzakc.width * (ridzakScale+0.1)));
				ridzakc.updateHitbox();
				ridzakc.scrollFactor.x = 0;
				ridzakc.scrollFactor.y = 0;
				ridzakc.color = FlxColor.BLACK;
				if (FlxG.save.data.minusUnlocked)
					ridzakc.color = FlxColor.WHITE;
				ridzakc.dance();
				add(ridzakc);
				...
				ridzaka = new Character(ridzakx-110, ridzaky-100, 'ridzak-palpha', true);
				ridzaka.setGraphicSize(Std.int(ridzaka.width * (ridzakScale+0.1)));
				ridzaka.updateHitbox();
				ridzaka.scrollFactor.x = 0;
				ridzaka.scrollFactor.y = 0;
				ridzaka.color = FlxColor.BLACK;
				if (FlxG.save.data.minusUnlocked != true)
					ridzaka.color = FlxColor.WHITE;
				ridzaka.dance();
				add(ridzaka);
				...
				else if (FlxG.save.data.currentRidzak == 'alpha')
					{
						isRidzak = 3;
						ridzaka.visible = true;
					}
			}
		function changeRidZak(change:Int)
			{
				isRidzak += change;
				if (isRidzak >= 4)
					isRidzak = 0;
				...
				if (isRidzak == 0)
					...
					{
						FlxG.save.data.currentRidzak = 'alpha';
						ridzaka.visible = true;
					}
				...
				FlxTween.tween(ridzaka, {y: ridzaka.y + 15, alpha: 1}, 0.07);
			}
}